...
async function seed() {
  ...
  const patents = [
    {
      ...
      description: 'IoT-enabled dual-axis clinostat/RPM platform with a novel Hybrid mode for controlled microgravity simulation across planetary gravity profiles.',
      innovation: 'Hybrid Clinostat–RPM mode, Microgravity-on-Chip concept, IoT wireless control with real-time telemetry, and universal sample holding module for diverse experimental formats.',
    ...
    },
    {
      ...
      status: 'search_report',
      description: 'Portable LED-based autofluorescence microscope with embedded on-edge AI pipeline for non-invasive, label-free oral cancer screening at point of care.',
      innovation: 'Integrated AFI hardware with on-edge AI inference running the full denoising to classification pipeline locally on Raspberry Pi 5, eliminating cloud dependency.',
  ...
  // Update existing patents that still have old long descriptions
  const patentUpdates = [
    { title: 'Automated Multi-Modal Microgravity-on-a-Chip Simulation Platform', description: 'IoT-enabled dual-axis clinostat/RPM platform with a novel Hybrid mode for controlled microgravity simulation across planetary gravity profiles.', innovation: 'Hybrid Clinostat\u2013RPM mode, Microgravity-on-Chip concept, IoT wireless control with real-time telemetry, and universal sample holding module for diverse experimental formats.' },
    { title: 'Label-free Autofluorescence Imaging Device & AI-based Screening Method for Oral Cancer', description: 'Portable LED-based autofluorescence microscope with embedded on-edge AI pipeline for non-invasive, label-free oral cancer screening at point of care.', innovation: 'Integrated AFI hardware with on-edge AI inference running the full denoising to classification pipeline locally on Raspberry Pi 5, eliminating cloud dependency.' },
  ];
  const updatePatentStmt = db.prepare('UPDATE patents SET description = ?, innovation = ? WHERE title = ? AND length(description) > 200');
  for (const p of patentUpdates) {
    updatePatentStmt.run(p.description, p.innovation, p.title);
  }
  console.log('✅ Patent descriptions shortened');
