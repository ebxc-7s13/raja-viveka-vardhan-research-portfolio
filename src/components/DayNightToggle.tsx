...
// Theme context so DotGrid can read day mode
const ThemeContext = createContext<{ isDay: boolean }>({ isDay: false });
...
function Sun() {
  return (
    <div className="absolute top-0 right-0 w-64 h-64 pointer-events-none" style={{ zIndex: 1 }}>
...
export default function DayNightToggle({ children }: DayNightToggleProps) {
  ...
  return (
    <ThemeContext.Provider value={{ isDay }}>
      {/* Background — z-0, below DotGrid */}
      <div
        className="fixed inset-0 transition-colors duration-700 pointer-events-none"
        style={{ zIndex: 0, backgroundColor: isDay ? '#E8F4FD' : '#000000' }}
      />

      {/* Day mode nature elements — z-[3], above DotGrid trail */}
      {isDay && (
      ...
      <button
        onClick={toggle}
        className="fixed top-6 right-24 z-50 w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
        style={{
      ...
      {/* Page content — z-1, above background but BELOW DotGrid trail */}
      <div style={{ position: 'relative', zIndex: 1 }}>
        {isDay ? (
          <div className="day-theme">{children}</div>
        ) : (
          children
        )}
      </div>
